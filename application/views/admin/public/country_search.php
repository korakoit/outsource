        	...
        	<div class="nest" id="tagInputClose">
				...
				<form  class="form-horizontal" method="post" action="<?=site_url('public_country/search')?>">
                        	<div class="form-group">
                            	...
                            	<div class="col-sm-1"><input type="submit" name="submit" value="搜索" onclick="location.href='<?=site_url('public_country/sear');?>'" class="ntn btn-success"></div>
                                &nbsp; <div class="col-sm-1"><button type="button" name="submit" onclick="location.href='<?=site_url('public_country/add');?>'"  class="ntn btn-success">新增</button></div>
                            </div>
			</form></div>
                	...
                	<table class="table-striped footable-res footable metro-blue">
                    	<thead>
                       	...
                       	<?php foreach($info->result() as $v){
                       	    $edit = site_url('public_country/edit/'.$v->id);
                       	    $delete = site_url('public_country/delete/'.$v->id); 
                       	?>
                       	<tbody>
                       		<tr>
                       		       <td><?= $v->id?></td>
                       		...
                       		<td><input class="btn" type=submit name="edit" onclick="location.href='<?=$edit?>'" value="编辑">&nbsp;&nbsp;&nbsp;<input type="submit" class="btn" name="delete" onclick="location.href='<?=$delete?>'" value="删除"></td>
                       		</tr>
                          <?php }?>
                     	</tbody>
                     	<tfoot>
